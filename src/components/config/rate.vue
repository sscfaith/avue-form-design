<template>
  <div>
    ...
    <el-form-item label="最大星数">
      <el-input-number v-model="data.max" controls-position="right" placeholder="最大星数"></el-input-number>
    </el-form-item>
    <el-form-item label="是否显示文本">
      <el-switch v-model="data.showText"></el-switch>
    </el-form-item>
    <el-form-item label="自定义文本" v-if="data.showText">
      <el-tag :key="index" v-for="(tag,index) in data.texts" closable @close="handleTextClose(tag)">{{tag}}</el-tag>
      ...
      <el-button v-else @click="showTextInput" size="mini" icon="el-icon-plus" circle
                 style="margin-left: 5px;"></el-button>
    </el-form-item>
    <el-form-item label="自定义颜色">
      <el-tag :key="index" v-for="(tag,index) in data.colors" closable @close="handleColorClose(tag)"
              :style="{color: tag}">{{tag}}
      </el-tag>
      <el-color-picker v-model="colorValue" size="mini"
                       @change="handleColorConfirm" class="color-picker"></el-color-picker>
    </el-form-item>
